...
public class WeaponScriptEnemy : MonoBehaviour {
	...
	public void DoDropWeapon (Vector3 playerPos) {
		...
		power = 125;
		Vector3 mousePos = enemy.transform.position;
		dropDir = mousePos;
	...
	public void DoTakeWeapon (GameObject obj, GameObject player) {
	...
	public IEnumerator DoShoot(GameObject master) {
		...
		while (ammo != 0) {
			if (ammo != -1)
				ammo -= 1;
			dir = (target.transform.position - enemy.transform.position).normalized;
